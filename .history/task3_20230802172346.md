...
### How complex does my data, statistical test, or model need to be?

It must be complex enough to meet the needs of your project. There is no explicit minimal complexity for any of these items. However, the methods must support the project's goals and the data must be appropriate for the chosen methods which could indirectly impose a minimal complexity. For example, a supervised model requires two variables.
