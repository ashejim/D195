...
- The metric(s) generated from that test (e.g., a t-stat) from which probability (the $p$is derived.

The alpha value (denoted α; usually 1% or 5%) that will be used to determine statistical significance (e.g., if α = .05 and p-value = .025 then the null hypothesis will be rejected and there is sufficient evidence to support the hypothesis).  
