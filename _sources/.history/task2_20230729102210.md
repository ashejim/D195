...
There is no explicit minimal complexity for either. However, the data must meet the needs of the research question and the method must be appropriate for both the data and the research question which may indirectly set some minimal r. For example, For the data there is no minimal required number of elements 
