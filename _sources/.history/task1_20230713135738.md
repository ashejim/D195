...
While the rubric explicitly requires "statistical significance," not all non-descriptive rigorous data analysis methods have a known probability space from which to derive a $p-\text{value}$, e.g., many common machine learning models; but the use of such models is acceptable. 
